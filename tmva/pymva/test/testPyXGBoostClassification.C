...
int testPyXGBoostClassification(){
   // Get data file
   ...
   UInt_t ret;
   ret = gSystem->Exec("echo '"+pythonSrc+"' > generateXGBoostModelClassification.py");
   if(ret!=0){
       std::cout << "[ERROR] Failed to write python code to file" << std::endl;
       return 1;
   }
   ret = gSystem->Exec("python generateXGBoostModelClassification.py");
   if(ret!=0){
   ...
   TFile* outputFile = TFile::Open("ResultsTestPyXGBoostClassification.root", "RECREATE");
   TMVA::Factory *factory = new TMVA::Factory("testPyXGBoostClassification", outputFile,
      "!V:Silent:Color:!DrawProgressBar:AnalysisType=Classification");

   // Load data
   TMVA::DataLoader *dataloader = new TMVA::DataLoader("datasetTestPyXGBoostClassification");
   ...
   factory->BookMethod(dataloader, TMVA::Types::kPyXGBoost, "PyXGBoost",
      "!H:!V:VarTransform=D,G:FilenameModel=kerasModelClassification.h5:FilenameTrainedModel=trainedXGBoostModelClassification.h5:NumEpochs=10:BatchSize=32:SaveBestOnly=false:Verbose=0");
   ...
   reader->BookMVA("PyXGBoost", "datasetTestPyXGBoostClassification/weights/testPyXGBoostClassification_PyXGBoost.weights.xml");
   ...
   for(UInt_t i=0; i<numEvents; i++){
      signal->GetEntry(i);
      meanMvaSignal += reader->EvaluateMVA("PyXGBoost");
      background->GetEntry(i);
      meanMvaBackground += reader->EvaluateMVA("PyXGBoost");
   }
...
int main(){
   int err = testPyXGBoostClassification();
   return err;
}
