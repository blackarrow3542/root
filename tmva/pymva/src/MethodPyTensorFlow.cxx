...
#include <Python.h>
#include "TMVA/MethodPyTensorFlow.h"
...
REGISTER_METHOD(PyTensorFlow)

ClassImp(MethodPyTensorFlow)

MethodPyTensorFlow::MethodPyTensorFlow(const TString &jobName, const TString &methodTitle, DataSetInfo &dsi, const TString &theOption)
   : PyMethodBase(jobName, Types::kPyTensorFlow, methodTitle, dsi, theOption) {
   fNumEpochs = 10;
   fBatchSize = 100;
   fVerbose = 1;
...
MethodPyTensorFlow::MethodPyTensorFlow(DataSetInfo &theData, const TString &theWeightFile)
    : PyMethodBase(Types::kPyTensorFlow, theData, theWeightFile) {
   fNumEpochs = 10;
   fBatchSize = 100;
   fVerbose = 1;
   fContinueTraining = false;
...
MethodPyTensorFlow::~MethodPyTensorFlow() {
}

Bool_t MethodPyTensorFlow::HasAnalysisType(Types::EAnalysisType type, UInt_t numberClasses, UInt_t) {
...
void MethodPyTensorFlow::DeclareOptions() {
   DeclareOptionRef(fFilenameModel, "FilenameModel", "Filename of the initial TensorFlow model");
   DeclareOptionRef(fFilenameTrainedModel, "FilenameTrainedModel", "Filename of the trained output TensorFlow model");
   DeclareOptionRef(fBatchSize, "BatchSize", "Training batch size");
   DeclareOptionRef(fNumEpochs, "NumEpochs", "Number of training epochs");
   DeclareOptionRef(fVerbose, "Verbose", "TensorFlow verbosity during training");
...
void MethodPyTensorFlow::ProcessOptions() {
   ...
   SetupTensorFlowModel(fContinueTraining);
}

void MethodPyTensorFlow::SetupTensorFlowModel(bool loadTrainedModel) {
   /*
   ...
   PyRunString("saver = tf.train.import_meta_graph('"+filenameLoadModel+"')\n"
...
void MethodPyTensorFlow::Init() {
   ...
   PyRunString("import tensorflow as tf\n"
               "sess = tf.Session()\n", "Import TensorFlow failed");

   // Set flag that model is not setup
   fModelIsSetup = false;
}

void MethodPyTensorFlow::Train() {
   ...
   if (!fSaveBestOnly) {
      PyRunString("saver = tf.train.Saver()\n"
                  "saver.save(sess,'"+fFilenameTrainedModel+"')",
...
void MethodPyTensorFlow::TestClassification() {
    MethodBase::TestClassification();
}

Double_t MethodPyTensorFlow::GetMvaValue(Double_t *errLower, Double_t *errUpper) {
...
std::vector<Double_t> MethodPyTensorFlow::GetMvaValues(Long64_t firstEvt, Long64_t lastEvt, Bool_t) {
   // Check whether the model is setup
   // NOTE: Unfortunately this is needed because during evaluation ProcessOptions is not called again
   if (!fModelIsSetup) {
      // Setup the trained model
...
std::vector<Float_t>& MethodPyTensorFlow::GetRegressionValues() {
   // Check whether the model is setup
   // NOTE: unfortunately this is needed because during evaluation ProcessOptions is not called again
   if (!fModelIsSetup){
...
std::vector<Float_t>& MethodPyTensorFlow::GetMulticlassValues() {
   // Check whether the model is setup
   // NOTE: unfortunately this is needed because during evaluation ProcessOptions is not called again
   if (!fModelIsSetup){
      // Setup the model and load weights
...
void MethodPyTensorFlow::ReadModelFromFile() {
}

void MethodPyTensorFlow::GetHelpMessage() const {
// typical length of text line:
//          "|--------------------------------------------------------------|"
   Log() << Endl;
   Log() << "TensorFlow is a high-level API for the Theano and Tensorflow packages." << Endl;
   Log() << "This method wraps the training and predictions steps of the TensorFlow" << Endl;
   Log() << "Python package for TMVA, so that dataloading, preprocessing and" << Endl;
   Log() << "evaluation can be done within the TMVA system. To use this TensorFlow" << Endl;
   Log() << "interface, you have to generate a model with TensorFlow first. Then," << Endl;
